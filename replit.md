...
- **Region must be TIGHT.** NCC compares the whole region crop, so a box much larger than the logo gets swamped by the changing picture around it and the match score collapses (a clearly-present logo can score ~0.15–0.3, below a 0.6 threshold → false `logo_missing`/DOWN). The UI shows the live `match_score` on Preview after a reference is saved so the operator can shrink the box around just the logo until the score sits well above the threshold, then re-capture. This is a setup/tuning concern, not a code bug — a loose region is the usual cause of a false "Expected logo not detected".
