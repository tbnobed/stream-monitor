...
2. **Program alive?** When the feed loads, the **same WHEP probe** also inspects the *already-decoded* WebRTC frames (no extra connection, no RTMP) with numpy: per-frame mean luminance for **black**, **time-spaced** mean abs diff for **freeze** (each frame is compared against a reference grabbed ≥`FREEZE_INTERVAL_SECONDS` (1.0s) earlier, NOT the immediately-preceding frame — otherwise a low-motion live shot like a locked-off camera on a speaker reads as frozen because per-frame change is tiny), and audio RMS→dBFS for **silence**. A frozen screen → **DOWN** immediately; a **black** screen → **DOWN** only after a grace period (see below); audio flowing but **no video being sent** → **WARNING** (`No video frames`); sustained silence → **WARNING**. **Black-screen grace:** many channels load a black slate for a minute or two before the show starts, so a black screen escalates to DOWN only after it has been **continuously** black for `BLACK_GRACE_SECONDS` (env var, default `300` = 5 min; set `0` to alert immediately). The worker stamps `Device.black_since` on the first black cycle and clears it the moment the picture returns or the feed drops, so each fresh black slate gets a full window. Because a black screen is also a frozen one, the freeze verdict is suppressed during the same grace window (otherwise the slate would just trip "Frozen frame"). This catches a device stuck on a black/frozen/silent program that still delivers WebRTC frames (which would otherwise read HEALTHY). Because it analyses the exact picture the operator sees over WebRTC, it works **everywhere** (Replit dev sandbox *and* LAN) — no RTMP reachability required.
...
- **Setup (operator)**: On `/devices`, edit a device → "Logo Presence Monitoring" → toggle on, then "Preview region" to grab a live snapshot and **click & drag directly on the snapshot to draw the monitored region** (the X/Y + width/height **percentage** inputs stay in sync and remain for fine-tuning; defaults target a top-right logo). The drag handlers live in `LogoMonitorSection` (pointer events on the snapshot wrapper → `getBoundingClientRect` → 0–100% → `form.setValue` on the four region fields). Then "Capture & save reference". Saving grabs a live WHEP frame, stores a 48×48 grayscale template (base64, internal — never exposed via API), the region (fractions), and the match threshold on the Device, and enables the check.
...
- `create_all` does NOT alter existing tables. New columns on an existing model must be added with `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`. Startup in `main.py` runs idempotent `ALTER TABLE devices ADD COLUMN IF NOT EXISTS` statements (for `ip_address`, `remote_config`, the logo columns `logo_check_enabled`, `logo_region`, `logo_match_threshold`, `logo_template`, `logo_missing_since`, and the black-grace timer `black_since`) right after `create_all`, so existing deployments self-heal on boot. When you add a new column to an existing model, add a matching `ADD COLUMN IF NOT EXISTS` line there.
