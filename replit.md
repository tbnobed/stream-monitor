...
- **Setup (operator)**: `/devices` → edit → "Logo Presence Monitoring" → toggle on → "Preview region" to grab a live snapshot → **click & drag on the snapshot to draw the region** (X/Y/W/H % inputs stay in sync for fine-tuning) → "Capture & save reference". Saving stores a 48×48 grayscale template (base64, internal — never in API), the region (fractions), and the threshold on the Device.
...
- **Region must be TIGHT**: NCC compares the whole crop, so a loose box gets swamped by the changing picture and the score collapses (loose ≈0.15 vs tight ≈1.0 across a scene change). Defaults are a small top-right box (x:85%, y:4%, w:9%, h:6%); the sliding matcher tolerates small misalignment but the operator must shrink the box to the logo. Preview shows the live `match_score` so you can set the threshold below it.
...
- **Capture endpoint**: `POST /devices/{id}/logo/reference` `{region, save, threshold?}` → `snapshot`/`crop` data URLs + `match_score` (live NCC vs the *already-saved* reference, `null` if none). `save=true` persists + enables. Region/threshold bounds-validated (fractions 0–1).
